...
const createCanonical = (options, path) => {
  ...
  if (options.canonical === 'auto') {
    ...
    canonicalUrl = options.canonical.startsWith('http')
      ? options.canonical
...
module.exports = function (moduleOptions) {
  ...
  this.nuxt.hook('build:before', () => {
    ...
    this.options.head.meta = createMeta(
      options,
      this.options.head.meta,
      template
    )
    this.options.head.link = createCanonical(options, '/').concat(
    ...
    this.addPlugin({
      src: path.resolve(__dirname, '../templates/seo_plugin.tpl'),
